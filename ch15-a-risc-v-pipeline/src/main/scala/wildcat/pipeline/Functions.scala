...
// The heart of Wildcat: the datapath pieces written as Scala FUNCTIONS that
// return hardware (a lighter form than modules). decode, alu, compare, getImm,
// registerFile, selectLoadData, and getWriteData are composed by the pipeline.
object Functions {

  // Decode an instruction into a DecodedInstr bundle (opcode -> control flags).
  ...
  // The register file. Uses SyncReadMem (WriteFirst gives read-during-write
  // forwarding). Register 0 always reads as 0. debugRegs mirrors the state for
  // testing only (ignored at synthesis). Returns (rs1Val, rs2Val, debugRegs).
