// Generalized, ready/valid FIFOs, parameterized by a Chisel data type.
// An abstract base + a shared FifoIO let several implementations be
// interchangeable (inheritance, Chapter 10).
package fifo

import chisel3._
import chisel3.util._

// The shared interface: enqueue (Flipped, since DecoupledIO is producer-side)
// and dequeue DecoupledIO ports.
...
// Bubble FIFO: a chain of one-word buffers (max throughput: 1 word / 2 cycles).
...
// Double buffer: each stage holds TWO words (data + shadow), so it can stay
// ready when full -> half the stages, half the restart latency.
class DoubleBufferFifo[T <: Data](gen: T, depth: Int) extends Fifo(gen: T, depth: Int) {
  ...
  private class DoubleBuffer[T <: Data](gen: T) extends Module {
    ...
    switch(stateReg) {
      is(empty) {
        when(io.enq.valid) { stateReg := one; dataReg := io.enq.bits }
      }
      is(one) {
        when(io.deq.ready && !io.enq.valid) { stateReg := empty }
        when(io.deq.ready && io.enq.valid) { stateReg := one; dataReg := io.enq.bits }
        when(!io.deq.ready && io.enq.valid) { stateReg := two; shadowReg := io.enq.bits }
      }
      is(two) {
        when(io.deq.ready) { dataReg := shadowReg; stateReg := one }
      }
    }
...
// Circular-buffer FIFO in a register file, with read/write pointers.
class RegFifo[T <: Data](gen: T, depth: Int) extends Fifo(gen: T, depth: Int) {

  // A wrapping counter; returns (current, next) so callers can look ahead.
...
// Same circular buffer, but backed by on-chip SyncReadMem (for larger FIFOs).
// The synchronous read adds one cycle of latency, handled with an output reg.
class MemFifo[T <: Data](gen: T, depth: Int) extends Fifo(gen: T, depth: Int) {
  ...
  io.deq.bits := data
  when(doWrite) { mem.write(writePtr, io.enq.bits) }
  ...
  when(io.deq.fire) { outputValidReg := false.B }
...
// Compose a memory FIFO with a double-buffer stage to decouple the memory read
// path from the output (helps timing).
