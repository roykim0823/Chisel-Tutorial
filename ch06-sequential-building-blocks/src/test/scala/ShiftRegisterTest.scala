...
class ShiftRegisterTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "ShiftRegister" should "delay by 4 cycles" in {
  ...
  "ShiftRegister parallel out" should "assemble a word" in {
    test(new ShiftRegister) { dut =>
  ...
  "ShiftRegister parallel load" should "shift out a loaded word" in {
    test(new ShiftRegister) { dut =>
      dut.io.d.poke(0.U)
